...
' Contributors
'
' Graham Mayoor, graham@gMayoor.com, www.gMayoor.com
...
' completed my checklist, here we are! Now my hope is that some other tired soul Mayo
...
Public Enum calFirstWeekOfYear      'Controls how the week numbers are calculated and displayed
    FirstJan1 = 1                   'The week with Enero 1st is always counted as week 1
    FirstFourDays = 2               'The first week in Enero that has at least four days in it is
                                        'counted as week 1. This calculation will change depending
                                        'on the setting used for first day of the week. The ISO
                                        'standard is calculating week 1 as the first week in Enero
                                        'with four days with Monday being the first day of the week.
    FirstFullWeek = 3               'The first week in Enero with a full week is counted as week 1.
...
Private lblDateSatColor As Long             'Font color of Sabado date labels
Private lblDateSunColor As Long             'Font color of Domingo date labels
...
'       Diciembre 2024, it will keep right on going to 2025 and beyond (and those
...
'   SabadoFontColor (Long) - Sets the font color of Sabado date labels.
'   DomingoFontColor (Long) - Sets the font color of Domingo date labels.
...
Public Function GetDate(Optional SelectedDate As Date = 0, _
    Optional FirstDayOfWeek As calDayOfWeek = Domingo, _
    ...
    Optional SabadoFontColor As Long = 0, _
    Optional DomingoFontColor As Long = 0, _
    ...
    lblDateSatColor = SabadoFontColor
    lblDateSunColor = DomingoFontColor
...
Private Sub InitializeUserform(SelectedDate As Date, MinimumDate As Date, MaximumDate As Date, _
    ...
    If MinimumDate <= 0 Then
        MinDate = CDate("1/1/2020")
    Else
        MinDate = MinimumDate
    End If
    If MaximumDate = 0 Then
        MaxDate = CDate("12/31/2099")
    Else
        MaxDate = MaximumDate
    End If
    ...
    For i = 1 To 7
        Me("lblDay" & CStr(i)).Caption = Choose(TempDayOfWeek, "Dom", "Lun", "Mar", "Mie", "Jue", "Vie", "Sab")
...
Private Sub scrlMonth_Change()
    ...
    'If user goes down one month from Enero, scroll bar will have value of
    '0. In this case, reset scroll bar back to Diciembre and decrement year
    'by 1.
    If scrlMonth.value = 0 Then
        ...
        End If
        Call SetMonthCombobox(cmbYear.value, scrlMonth.value)
    'If user goes up one month from Diciembre, scroll bar will have value of
    '13. Reset to Enero and increment year.
...
Private Sub SetMonthCombobox(YearIn As Long, MonthIn As Long)
    ...
    If YearIn = YearMinDate And YearIn = YearMaxDate Then
        For i = MonthMinDate To MonthMaxDate
            cmbMonth.AddItem Choose(i, "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre")
        Next i
    ...
    Else
        cmbMonth.List = Array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre")
        cmbMonth.ListIndex = MonthIn - 1
    End If
...
Private Sub SetDays(MonthIn As Long, YearIn As Long, Optional DayIn As Long)
    ...
    Dim SabadoIndex As Long           'Column index of Sabados. Used to set color of Sabado labels, if applicable
    Dim DomingoIndex As Long             'Column index of Domingos
    ...
    'Find previous month and next month. Handle Enero
    'and Diciembre appropriately
    PrevMonth = MonthIn - 1
    ...
    'in current month, and index of Sabado and Domingo relative to first weekday
    ...
    If StartWeek = 1 Then DomingoIndex = 1 Else DomingoIndex = 9 - StartWeek
    SabadoIndex = 8 - StartWeek
    ...
    For i = 1 To 6 'Rows
        ...
        'in Diciembre can be calculated incorrectly, so we want to default
        'to Enero 1st instead, which is always correct
        ElseIf i = 1 Then
        ...
        'for last week in Diciembre when it should be week 1 of new year. If we get
        '53, but Enero 1st resides in the week we are calculating (any time the
        'first day of the week is greater than Dec 25th), we want to calculate based
        'off Enero 1st, instead of date in Diciembre.
            ...
            With lblControl
                ...
                Else
                    ...
                    Else 'Within bounds. Enable and set colors
                        ...
                        ElseIf j = SabadoIndex Then
                            .ForeColor = lblDateSatColor
                        ElseIf j = DomingoIndex Then
...
' -Diciembre 9999 generates an error when trying to calculate last day of month,  because
'   Enero 10000 is not a valid date in Excel
